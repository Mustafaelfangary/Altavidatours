...
export function TravelOKFooter() {
  ...
  return (
    <footer className="bg-gradient-to-b from-[#0B2E4F] to-[#061A2A] text-white border-t border-gray-800/30">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
          {/* About Column */}
          <div>
            <h3 className="text-xl font-bold mb-4 text-[#D4AF37]">About Altavida Tours.com</h3>
            ...
            <ul className="space-y-2">
              {quickLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-white/80 hover:text-[#D4AF37] hover:translate-x-1 inline-block transition-all duration-200 text-sm"
                  >
                    {link.label}
                  </Link>
                </li>
          ...
          <div>
            ...
            <form onSubmit={handleNewsletterSubmit} className="space-y-3">
              <input
                ...
                required
                className="w-full px-4 py-2 rounded-lg border border-white/20 bg-white/10 text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-[#D4AF37] transition-all"
              />
                <button
                type="submit"
                className="w-full px-4 py-2 rounded-lg bg-gradient-to-r from-[#0B2E4F] to-[#D4AF37] text-white font-semibold transition-all duration-300 hover:shadow-lg"
              >
