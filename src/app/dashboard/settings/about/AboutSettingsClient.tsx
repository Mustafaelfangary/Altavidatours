...
export default function AboutSettingsClient() {
  ...
  const loadContent = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/dashboard/about');
  ...
  const initializeContent = async () => {
    try {
      setSaving(true);
      const response = await fetch('/api/dashboard/about', { method: 'POST' });
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading about settings...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <Button onClick={initializeContent} disabled={saving}>
              <RefreshCw className="w-4 h-4 mr-2" />
              Initialize About Content Structure
            </Button>
          </CardContent>
        </Card>
      ) : (
      ...
      <MediaLibrarySelector
        ...
        title="Select Media for About Page"
      />
    </div>
  );
}
