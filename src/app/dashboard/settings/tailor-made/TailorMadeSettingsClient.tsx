...
export default function TailorMadeSettingsClient() {
  ...
  const loadContent = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/dashboard/tailor-made');
  ...
  const initializeContent = async () => {
    try {
      setSaving(true);
      const response = await fetch('/api/dashboard/tailor-made', { method: 'POST' });
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading tailor-made settings...</p>
        </div>
      </div>
    );
  }
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <Button onClick={initializeContent} disabled={saving}>
              <RefreshCw className="w-4 h-4 mr-2" />
              Initialize Tailor-Made Content Structure
            </Button>
          </CardContent>
        </Card>
      ) : (
