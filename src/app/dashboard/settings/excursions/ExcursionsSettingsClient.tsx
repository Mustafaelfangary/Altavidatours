...
import { Save, RefreshCw, Landmark, Mountain, Sparkles } from 'lucide-react';
...
export default function ExcursionsSettingsClient() {
  ...
  const handleSave = async () => {
    try {
      ...
      // Get all content blocks to find their metadata
      const allBlocks = Object.values(content).flat();

      // Save each edited value with proper metadata
  ...
  const sections = [
    ...
    { id: 'featured', label: 'Featured Tours', icon: '⭐', description: 'Featured section titles' },
    { id: 'all', label: 'All Excursions', icon: '📂', description: 'All excursions section and tabs' },
    { id: 'gallery', label: 'Gallery', icon: '🖼️', description: 'Gallery section titles' },
    { id: 'cta', label: 'Call to Action', icon: '📢', description: 'Bottom CTA section' },
  ];
                    ...
                    <div key={block.id} className="border rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="font-medium capitalize">
                          {block.key.replace(/excursions_|_/g, ' ').trim()}
                        </span>
                      </div>
                      {renderField(block)}
                    </div>
