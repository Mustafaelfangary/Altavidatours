...
export default function BeachPackagesPage() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const res = await fetch('/api/packages?category=beaches');
        if (res.ok) {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-12 text-center">
          <h1 className="text-5xl font-bold mb-4 bg-linear-to-r from-amber-400 to-yellow-300 bg-clip-text text-transparent">
            {t('beachesAndSeas') || 'Beach & Sea Packages'}
          </h1>
          <p className="text-xl text-gray-300 max-w-2xl mx-auto">
            Relax and unwind on Egypt's beautiful beaches and experience the magic of the Red Sea.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-xl text-gray-400 mb-4">No beach packages available at the moment.</p>
            <Link href="/packages">
