...
export default function AdventurePackagesPage() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const res = await fetch('/api/packages?category=adventures');
        if (res.ok) {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-12 text-center">
          <h1 className="text-5xl font-bold mb-4 bg-linear-to-r from-amber-400 to-yellow-300 bg-clip-text text-transparent">
            {t('safariSnorkeling') || 'Adventure Packages'}
          </h1>
          <p className="text-xl text-gray-300 max-w-2xl mx-auto">
            Experience thrilling adventures through Egypt's deserts, Red Sea, and unique landscapes.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-xl text-gray-400 mb-4">No adventure packages available at the moment.</p>
            <Link href="/packages">
