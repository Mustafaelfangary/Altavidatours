...
import { ArrowRight, MapPin, Users, Clock, DollarSign, Crown } from 'lucide-react';
...
export default function LuxuryPackagesPage() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const res = await fetch('/api/packages?category=luxury');
        if (res.ok) {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-12 text-center">
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <h1 className="text-5xl font-bold bg-linear-to-r from-amber-400 to-yellow-300 bg-clip-text text-transparent">
              Luxury & Exclusive
            </h1>
            <Crown className="w-10 h-10 text-amber-400" />
          </div>
          <p className="text-xl text-gray-300 max-w-2xl mx-auto">
            Experience Egypt's most exclusive and luxurious travel experiences tailored for the discerning traveler.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {packages.map((pkg) => (
              <Link key={pkg.id} href={`/packages/${pkg.id}`}>
                <div className="bg-linear-to-b from-gray-800 to-gray-900 rounded-lg overflow-hidden hover:shadow-2xl transition-all duration-300 group cursor-pointer h-full border border-amber-400/30 hover:border-amber-400/60">
                  {pkg.image && (
                  ...
                  <div className="p-6">
                    ...
                    {pkg.price && (
                      <div className="flex items-center text-amber-300 mb-3 font-semibold">
          ...
          <div className="text-center py-12">
            <p className="text-xl text-gray-400 mb-4">No luxury packages available at the moment.</p>
            <Link href="/packages">
