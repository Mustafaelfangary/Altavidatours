...
export async function POST(request: Request) {
  try {
    ...
    // Verify daily tour and cabin availability
    ...
    if (!dailyTour || dailyTour.cabins.length === 0) {
      return new NextResponse('Invalid tour or cabin', { status: 400 });
    }

    if (dailyTour.availableDates.length === 0) {
      return new NextResponse('Selected dates are not available', { status: 400 });
    }

    // Calculate total price
    const cabin = dailyTour.cabins[0];
...
export async function PATCH(request: Request) {
  try {
    ...
    const mappedBooking = {
      ...updatedBooking,
      dahabiya: updatedBooking.dailyTour,
    };
